...
public class Raster2pgsqlReclassification extends Thread{
...
//	}
	...
	public Raster2pgsqlReclassification() throws IOException, SQLException, InterruptedException   { 
		...
		for(int j=0;j<FenetreReclassificationParametresClasses.getNomCouches().size();j++){
			...
			for (i=0;i<8;i++){		 
				...
				shp2pgsql[i++] = "-f";
				shp2pgsql[i++] = "rast";
				shp2pgsql[i++] = "./"+FenetreReclassificationParametresClasses.getNomCouches().get(j)+".tif";
				shp2pgsql[i++] = FenetreReclassificationParametresClasses.getNomCouches().get(j)+"_reclassifie";
		...
		for (int ai=0;ai<shptoo.size();ai++){
			...
			BufferedWriter varName = new BufferedWriter(new FileWriter(this.getRepertoireSQL()+"/"+FenetreReclassificationParametresClasses.getNomCouches().get(ai)+"_reclassifie.sql"));
		...
		try {
			new InsertSQLReclassification(this);
