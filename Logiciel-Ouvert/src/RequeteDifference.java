...
public class RequeteDifference extends JFrame implements ActionListener{
	/**
	 * 
	 */
	...
	public static void setComparaison(ArrayList<String> comparaison) {
		RequeteDifference.comparaison = comparaison;
	}


	ConnexionBaseDonnees connex;
	public RequeteDifference() throws SQLException {
		...
		nomCouches=FenetreDifference.getNomShape1();
		srids= FenetreDifference.getSrids();
			...
			//for(int j=0;j<nomCouches.size();j++){
				if (con!=null){
					try{
						...
						st.execute("drop table if exists diiiif_"+nomCouches.get(0));
						st.execute("drop table if exists diiiif_"+nomCouches.get(1));

						st.execute("create table diiiif_"+nomCouches.get(0)+" as select ST_Buffer("+srids.get(0)+", 0) as "+srids.get(0)+" from "+nomCouches.get(0));
						System.out.println("create table diiiif_"+nomCouches.get(0)+" as select ST_Buffer("+srids.get(0)+", 0) as geom_"+nomCouches.get(0)+" from "+nomCouches.get(0));

						st.execute("create table diiiif_"+nomCouches.get(1)+" as select ST_Buffer("+srids.get(1)+", 0) as "+srids.get(1)+" from "+nomCouches.get(1));
						System.out.println("drop table if exists diiiif_"+nomCouches.get(1));
						
						System.out.println("create table diff_"+nomCouches.get(0)+"_"+nomCouches.get(1)+" as select ST_SymDifference(a."+srids.get(0)+", b."+srids.get(1)+") from diiiif_"+nomCouches.get(0)+" a, diiiif_"+nomCouches.get(1)+" b");

						System.out.println("create table diff_"+nomCouches.get(0)+"_"+nomCouches.get(1)+" as select ST_SymDifference(a."+srids.get(0)+", b."+srids.get(1)+") from diiiif_"+nomCouches.get(0)+" a, diiiif_"+nomCouches.get(1)+" b");

						st.execute("create table diff_"+nomCouches.get(0)+"_"+nomCouches.get(1)+" as select ST_SymDifference(a."+srids.get(0)+", b."+srids.get(1)+") from diiiif_"+nomCouches.get(0)+" a, diiiif_"+nomCouches.get(1)+" b");
					//	System.out.println("create table diff_"+nomCouches.get(0)+"_"+nomCouches.get(1)+" as select ST_SymDifference(a."+srids.get(0)+", b."+srids.get(1)+") from diiiif_"+nomCouches.get(0)+" a, diiiif_"+nomCouches.get(1)+" b");


						st.execute("drop table diiiif_"+nomCouches.get(0));
						st.execute("drop table diiiif_"+nomCouches.get(1));
