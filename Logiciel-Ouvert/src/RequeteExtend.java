...
public class RequeteExtend extends JFrame implements ActionListener{
	/**
	 * 
	 */
	...
	public static void setComparaison(ArrayList<String> comparaison) {
		RequeteExtend.comparaison = comparaison;
	}


	ConnexionBaseDonnees connex;
	public RequeteExtend() throws SQLException {
		...
		nomCouches=FenetreExtend.getNomShape1();
		srids= FenetreExtend.getSrids();
			...
			for(int j=0;j<nomCouches.size();j++){
				if (con!=null){
					try{
						...
						st.execute("drop table if exists extend_"+nomCouches.get(j));

						st.execute("create table extend_"+nomCouches.get(j)+" as SELECT st_envelope(st_union(st_buffer("+srids.get(j)+",0))) from "+nomCouches.get(j));
