...
public class ConnexionTraitement extends JFrame{
	...
	public static void setGeomms2(ArrayList<String> geomms2) {
		ConnexionTraitement.geomms2 = geomms2;
	}
	...
	public static void setArrayy2(ArrayList<String> arrayy2) {
		ConnexionTraitement.arrayy2 = arrayy2;
	}
	...
	public static void setArrayy(ArrayList<String> arrayy) {
		ConnexionTraitement.arrayy = arrayy;
	}

	public ConnexionTraitement(){
		...
		try{
			...
			if (con!=null){
				try{
					...
					arrayy.removeAll(RequeteSurface.getShapes1());
					geomms.removeAll(RequeteSurface.getSrids1());
				}
		...
		System.out.println("bla3 "+arrayy3.size());
		for(String bla:arrayy3){
			System.out.println(bla);
		}



		arrayy3.removeAll(RequeteSurface.getShapes1());
		...
		if(arrayy3.size()!=0){
			FenetreTraitement fene = new FenetreTraitement();
			fene.setVisible(true);
		}
		else{
			System.out.println("Les tailles pour toutes les couches ont été calculées");
		}
		System.out.println("bla "+arrayy.size());
		for(String bla:arrayy){
			System.out.println(bla);
		}
	...
	public static void setGeomms(ArrayList<String> geomms) {
		ConnexionTraitement.geomms = geomms;
	}
