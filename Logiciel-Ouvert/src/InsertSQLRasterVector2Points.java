...
public class InsertSQLRasterVector2Points {

	ConnexionBaseDonnees connexion;
	Raster2pgsqlVector2Points shp2;
	...
	public InsertSQLRasterVector2Points(Raster2pgsqlVector2Points shp2) throws IOException, SQLException{
		...
		for(int j=0;j<RequeteRasterVector2Points.getNomRaster2().size();j++){
			...
			for (i=0;i<=10;i++){
				...
				insertSQL[i++]="-f";
				insertSQL[i++]="./"+RequeteRasterVector2Points.getNomRaster2().get(j)+"_densite.sql";
			}
			for (i=0;i<=10;i++){	
		...
		for(int b=0;b<RequeteRasterVector2Points.getNomRaster2().size();b++){
