...
public class ConnexionVisualiserVecteur extends JFrame{
	...
	public static void setGeomms(ArrayList<String> geomms) {
		ConnexionVisualiserVecteur.geomms = geomms;
	}
	...
	public static void setArrayy(ArrayList<String> arrayy) {
		ConnexionVisualiserVecteur.arrayy = arrayy;
	}

	public ConnexionVisualiserVecteur(){
		...
		try{
			...
			if (con!=null){
				try{
					...
					String query = "SELECT f_table_name as tables, f_geometry_column as geom FROM geometry_columns";
		...
		FenetreVisualiserVecteur fenvisvec = new FenetreVisualiserVecteur();
		fenvisvec.setVisible(true);
