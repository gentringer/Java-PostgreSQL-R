...
public class Raster2pgsqlCombinerRaster extends Thread{
...
//	}
	...
	public Raster2pgsqlCombinerRaster() throws IOException, SQLException, InterruptedException   { 
		...
		//for (String f:nameSQL){
			...
			for (i=0;i<8;i++){		 
				...
				shp2pgsql[i++] = "-f";
				shp2pgsql[i++] = "rast";
				shp2pgsql[i++] = "./"+RequeteCombinerRaster.getNomCoucheFinale();
				shp2pgsql[i++] = RequeteCombinerRaster.getNomCoucheFinale1();
		...
		for (int ai=0;ai<shptoo.size();ai++){
			...
			BufferedWriter varName = new BufferedWriter(new FileWriter(this.getRepertoireSQL()+"/"+RequeteCombinerRaster.getNomCoucheFinale1()+".sql"));
		...
		try {
			new InsertSQLRasterCombinaisonRaster(this);
