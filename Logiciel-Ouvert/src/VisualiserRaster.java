...
public class VisualiserRaster{
	...
	FenetreVisualiserRaster fenvis;

	private ArrayList<String> rasters = new ArrayList<String>();



	public VisualiserRaster(FenetreVisualiserRaster fenvis) throws IOException{

		try{

			this.fenvis=fenvis;
			rasters=FenetreVisualiserRaster.getNomShape1();
			...
			for(int i=0;i<rasters.size();i++){
				...
				code.addRCode("layers <- readGDAL(\"PG:host="+this.hote+" user="+this.nomUtilisateur+" dbname="+this.nomBase+" password="+this.motDePasse+" port="+this.port+" table="+rasters.get(i)+"\")");
				System.out.println("layers <- readGDAL(\"PG:host="+this.hote+" user="+this.nomUtilisateur+" dbname="+this.nomBase+" password="+this.motDePasse+" port="+this.port+" table="+rasters.get(i)+"\")");
        ...
        if(img!=null) g.drawImage(img.getImage(), 10, 30, plotter);
				...
				code.addRCode("plot(layers, main=\"Raster "+rasters.get(i)+"\",axes=FALSE,col=colorRampPalette(c(\"white\",\"yellow\", \"red\"))(30))");
				...
				jfra.setTitle("Affichage raster");
